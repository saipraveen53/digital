  ...
  return (
    ...
    >
      ...
      <View style={{ padding: 28, minHeight: 280, justifyContent: 'space-between' }}>
        <View style={{ flexDirection: 'row', justifyView: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <View style={{ width: 56, height: 56, borderRadius: 16, backgroundColor: showImage ? 'rgba(255,255,255,0.15)' : '#EBF4F1', alignItems: 'center', justifyView: 'center' }}>
        ...
        <View>
          <Text style={{ fontSize: 22, color: showImage ? '#ffffff' : '#111827', marginBottom: 12, fontWeight: '700' }}>
            {f.title}
          </Text>
...
const AnimatedBar = ({ d, index, color, isDesktop, start }: { d: { day: string, val: number }, index: number, color: string, isDesktop: boolean, start: boolean }) => {
  ...
  return (
    <View style={{ alignItems: 'center', width: `${100/7 - 2}%`, height: '100%', justifyView: 'flex-end' }}>
...
export default function Features() {
  ...
  // ✅ ANTI-DIRECTION SCROLL ANIMATION FOR BALLS (వ్యతిరేక దిశలో కదులుతాయి)
const ballStyle1 = useAnimatedStyle(() => ({
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FAF9F5' }}>
      {/* ✅ MULTIPLE COLORED BALLS BACKGROUND (As requested from image) */}
     <AnimatedRE.View style={[styles.blurredLiquidSphere1, ballStyle1]} />
      ...
      >
        <View style={{ maxWidth: 1200, alignSelf: 'center', width: '100%', paddingHorizontal: 24, paddingTop: 32, paddingBottom: 64, zIndex: 3 }}>
          ...
          <View style={{ flexDirection: isDesktop ? 'row' : 'column', gap: 24, marginBottom: 48 }}>
             ...
             <View style={{ flex: isDesktop ? 1 : undefined, width: '100%', backgroundColor: 'rgba(248, 246, 240, 0.9)', borderRadius: 32, padding: isDesktop ? 48 : 32, alignItems: 'center', borderWidth: 1, borderColor: '#F3F4F6' }}>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: 40, width: '100%', justifyView: 'center' }}>
                   {[
             ...
             <View style={{ flex: isDesktop ? 1.2 : undefined, width: '100%', backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: 32, padding: isDesktop ? 40 : 28, borderWidth: 1, borderColor: '#F3F4F6', shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 15, elevation: 2 }}>
                ...
                <View style={{ gap: 12 }}>
                   {recentLogs.map((log, i) => (
                      <View key={i} style={{ flexDirection: 'row', alignItems: 'center', justifyView: 'space-between', backgroundColor: '#F9FAFB', padding: 16, borderRadius: 16 }}>
          ...
          <View style={{ flexDirection: isDesktop ? 'row' : 'column', backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: 32, overflow: 'hidden', borderWidth: 1, borderColor: '#F3F4F6', marginBottom: 80, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 15, elevation: 2 }}>
             ...
             <View style={{ flex: isDesktop ? 1.2 : undefined, width: '100%', padding: isDesktop ? 48 : 28, justifyView: 'center' }}>
                <Text style={{ fontSize: 32, color: '#111827', marginBottom: 12, fontWeight: '800' }}>
                  Weekly Graph Trends
                </Text>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'flex-end', justifyView: 'space-between', height: 200 }}>
                   {weeklyData.map((d, i) => (
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 24, justifyView: 'space-between' }}>
             {features.map((f, i) => (
